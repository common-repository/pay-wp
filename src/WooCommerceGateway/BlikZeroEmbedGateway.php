...
final class BlikZeroEmbedGateway extends AbstractEmbedGateway {
	...
	public function process_payment( $order_id ) {
		...
		$blik_status = $order->get_meta( self::SESSION_BLIK_PAYED );
	...
	private function send_blik_payment( int $code, int $order_id ): bool {
		...
		$transaction_data = [
			'orderID'           => $order_id,
			'amount'            => apply_filters( 'wp_pay\payment\amount', number_format( $order->get_total( 'api' ), 2, '.', '' ), $order ),
	...
	public function refresh_blik_order_status(): void {
		...
		$started = $order->get_meta( self::SESSION_BLIK_TIME_STARTED ) ?? time();
