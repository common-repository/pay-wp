...
use WPPayVendor\WPDesk\Logger\Settings;
use WPPayVendor\WPDesk\Logger\SimpleLoggerFactory;
...
class Plugin extends AbstractPlugin implements LoggerAwareInterface, HookableCollection {
	...
	private const LOGGER_CHANNEL = 'wpdesk-autopay';
	...
	public function __construct( WPDesk_Plugin_Info $plugin_info ) {
		...
		$this->setLogger( ( new SimpleLoggerFactory( self::LOGGER_CHANNEL, $settings ) )->getLogger() );
	...
	public function hooks(): void {
		...
		$gateway              = new StandardPaymentGateway( $client_factory, $plugin_url, $this->logger );
		$blik_gateway         = new BlikZeroEmbedGateway( $client_factory, $plugin_url, $this->get_renderer(), $this->logger );
		$subscription_gateway = new SubscriptionGateway( $client_factory, $plugin_url, $this->logger );

		$this->autopay_gateway      = $gateway;
		$this->blik_gateway         = $blik_gateway;
		$this->subscription_gateway = $subscription_gateway;

		$api_handler = new APIHandler( $this->autopay_gateway, $this->subscription_gateway, $this->logger );
	...
	public static function has_credentials( string $currency = 'PLN' ): bool {
		...
		$settings             = new Settings();
		$settings->use_wp_log = false;
		$client_factory->set_settings( new StandardPaymentGateway( $client_factory, '', ( new SimpleLoggerFactory( self::LOGGER_CHANNEL, $settings ) )->getLogger() ) );
